...
<body onclick="deselect(event)" oncontextmenu="return false;">
    ...
    <div id="modal" onclick="closeModalOnOuterClick(event)">
        <div class="modal-content" onclick="event.stopPropagation()">
            ...
            <div id="host-fields">
                ...
                <div class="form-row"><label>Host:</label><input type="text" id="inp-host"></div>
                
                <div class="row-flex">
                    <div class="form-row"><label>Port:</label><input type="text" id="inp-port" value="22"></div>
                    <div class="form-row"><label>User:</label><input type="text" id="inp-user" value="root"></div>
                </div>
                ...
                <div class="form-row">
                    ...
                    <div class="radio-group">
                        <label class="radio-item"><input type="radio" name="auth" value="password" checked onchange="toggleAuth()"> Password</label>
                        <label class="radio-item"><input type="radio" name="auth" value="key" onchange="toggleAuth()"> SSH Key</label>
                    </div>
                </div>
                
                <div class="form-row"><label id="lbl-pass">Password / Key Path:</label><input type="password" id="inp-pass"></div>
            </div>
    ...
    <script>
        const SVGS = {
            ...
            monitor: '<svg class="icon-svg" viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="3" width="20" height="14" rx="2" ry="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/></svg>',
        ...
        function showContextMenu(e, type, id) {
            ...
            if (type === 'server') {
                items = `
                    <div class="ctx-item" onclick="connect('${safeId}'); hideContextMenu();">${SVGS.plus} Connect (New Tab)</div>
                    <div class="ctx-item" onclick="connect('${safeId}', 'current'); hideContextMenu();">${SVGS.refresh} Reconnect (Current)</div>
            ...
            sortedGroups.forEach(groupName => {
                ...
                if (!isCollapsed) {
                    grouped[groupName].forEach(srv => {
                        ...
                        sDiv.draggable = true;
                        sDiv.innerHTML = `
                            <span class="server-icon">${SVGS.monitor}</span>
                            <span class="server-name">${srv.name}</span>
                            <span class="server-sub">${srv.user}@${srv.host}</span>
                        `;
                        ...
                        sDiv.oncontextmenu = (e) => {
                            ...
                            showContextMenu(e, 'server', srv.id);
                        };
        ...
        function openModal(id = null) {
            ...
            document.getElementById('inp-host').value = srv?.host || '';
            document.getElementById('inp-port').value = srv?.port || '22';
            document.getElementById('inp-user').value = srv?.user || 'root';
            document.getElementById('inp-group').value = srv?.group || '';
            document.getElementById('inp-pass').value = srv?.password || '';
            const authVal = srv?.auth || 'password';
            document.querySelector(`input[name="auth"][value="${authVal}"]`).checked = true;
            toggleAuth();
        }
...
async def main(connection):
    ...
    async def handle_connect(request):
        ...
        if window:
            ...
            user, host, port, password, auth = srv.get("user", "root"), srv.get("host", ""), srv.get("port", "22"), srv.get("password", ""), srv.get("auth", "password")
            port_arg = f"-p {port}" if port and port != "22" else ""
            if auth == "key":
                cmd = f"ssh -o StrictHostKeyChecking=no {port_arg} -i {password} {user}@{host}\n" if password else f"ssh -o StrictHostKeyChecking=no {port_arg} {user}@{host}\n"
            else:
                if password:
                    import shlex
                    safe_pass = shlex.quote(password)
                    cmd = f"env IT2SSH_PASS={safe_pass} expect -c 'spawn ssh -o StrictHostKeyChecking=no {port_arg} {user}@{host}; expect \"*?assword:*\" {{ send \"$env(IT2SSH_PASS)\\r\" }}; interact'\n"
                else: cmd = f"ssh {port_arg} {user}@{host}\n"
    ...
    await site.start()
    await iterm2.tool.async_register_web_view_tool(connection, "SSH Manager", "com.benim.sshmanager", True, f"http://localhost:{free_port}/" )
